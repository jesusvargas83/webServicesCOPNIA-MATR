...
import co.gov.integrador.carpetaCiudadana.modelo.dto.SolicitudesSolicitudesPqrDTO;
...
public interface ServicioCiudadanoValidable {
    final Pattern PATRON_TIPOS_ID = Pattern.compile("^CC|NI|CE|PPT|PEP$");
    ...
    default SolicitudesDTO crearObjetoSolicitudesUsuario() {
        SolicitudesSolicitudesPqrDTO ssp = new SolicitudesSolicitudesPqrDTO();
        ssp.setFechaSolicitud("");
        ssp.setIdSolicitud("");
        ssp.setNomSolicitud("");
        ssp.setTextoRespuesta("");

        List<SolicitudesSolicitudesPqrDTO> lista = new ArrayList<SolicitudesSolicitudesPqrDTO>();
        lista.add(ssp);

        SolicitudesDTO s = new SolicitudesDTO();
        s.setSolicitudesPqr(lista);
        return s;
    }
}
